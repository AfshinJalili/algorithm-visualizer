...
const Player: React.FC<PlayerProps> = ({ className }) => {
  ...
  const { editingFile, shouldBuild } = useAppSelector(state => state.current);
  ...
  const resume = useCallback((wrap: boolean = false) => {
    pause();
    const canNext = next();
    if (canNext || (wrap && dispatch(setCursor(1)))) {
      const interval = 4000 / Math.pow(Math.E, speed);
      timerRef.current = window.setTimeout(() => resume(), interval);
      setPlaying(true);
    }
  }, [next, pause, speed, dispatch]);
  ...
  return (
    <div className={classes(styles.player, className)}>
      ...
      {playing ? (
        ...
        <Button icon={faPlay} primary onClick={() => resume(true)}>Play</Button>
      )}
