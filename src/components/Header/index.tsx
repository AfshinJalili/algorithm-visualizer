...
import { modifyTitle, setScratchPaper, setEditingFile, setExt, showErrorToast } from '../../reducers';
...
const Header: React.FC<HeaderProps> = ({ 
  ...
  const { scratchPaper, titles, files, lastFiles, editingFile, saved } = useAppSelector(state => state.current);
  ...
  const saveGist = () => {
    ...
    const save = (g: any) => {
      ...
      if (scratchPaper && scratchPaper.login) {
        if (scratchPaper.login === user.login) {
          ...
          return GitHubApi.forkGist(scratchPaper.gistId).then((forkedGist: any) => GitHubApi.editGist(forkedGist.id, g));
        }
      }
    ...
    save(gist)
      ...
      .then((newScratchPaper: any) => {
        ...
        dispatch(setEditingFile(newScratchPaper.files.find((file: any) => file.name === editingFile?.name)));
      ...
      <div className={styles.row}>
        ...
        <div className={styles.section}>
          <Button icon={permitted ? faSave : faCodeBranch} primary disabled={permitted && saved}
                  onClick={saveGist}>{permitted ? 'Save' : 'Fork'}</Button>
          {permitted &&
            <Button icon={faTrashAlt} primary onClick={deleteGist} confirmNeeded>Delete</Button>
          }
          ...
          <Button icon={faExpandArrowsAlt} primary
                  onClick={handleClickFullScreen}>Fullscreen</Button>
        </div>
      </div>
      <div className={styles.row}>
        <div className={styles.section}>
          ...
          <Button className={styles.btn_dropdown} icon={faStar}>
            ...
            <div className={styles.dropdown}>
              {languages.map(language => language.ext === ext ? null : (
                <ListItem key={language.ext} onClick={() => dispatch(setExt(language.ext))}
                          label={language.name}/>
              ))}
            </div>
          </Button>
