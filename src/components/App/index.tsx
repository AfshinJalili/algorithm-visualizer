...
const App: React.FC = () => {
  ...
  const { titles, description, saved, files, ext: currentExt } = useAppSelector(state => ({ 
    ...state.current, 
    ext: state.env.ext 
  }));
  ...
  const toggleHistoryBlock = useCallback((enable: boolean) => {
    if (enable) {
  ...
  const ignoreHistoryBlock = useCallback((process: () => void) => {
  ...
  const loadScratchPapers = useCallback(() => {
    const per_page = 100;
    const paginateGists = (page = 1, scratchPapers: any[] = []): any => GitHubApi.listGists({
      per_page,
      page,
      timestamp: Date.now(),
    }).then((gists: any) => {
      scratchPapers.push(...gists.filter((gist: any) => 'algorithm-visualizer' in gist.files).map((gist: any) => ({
        key: gist.id,
  ...
  const loadAlgorithm = useCallback((params: any, query: any) => {
